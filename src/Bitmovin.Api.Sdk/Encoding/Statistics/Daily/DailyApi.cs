...
namespace Bitmovin.Api.Sdk.Encoding.Statistics.Daily
{
    public class DailyApi
    {
        ...
        internal interface IDailyApiClient
        {
            ...
            [AllowAnyStatusCode]
            Task<Models.PaginationResponse<Models.DailyStatistics>> ListByDateRangeAsync([Path("from", Format = "yyyy-MM-dd")] DateTime? from, [Path("to", Format = "yyyy-MM-dd")] DateTime? to, [QueryMap(SerializationMethod = QuerySerializationMethod.Serialized)] IDictionary<String, Object> queryParams);
        }
